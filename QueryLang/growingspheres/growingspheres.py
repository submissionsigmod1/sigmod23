...
class GrowingSpheres:
    ...
    def ennemies_in_layer_(self, layer_shape='ring', radius=None, step=None, caps=None, n=1000, first_layer=False):
        ...
        if first_layer:
            layer = generate_ball(self.encoder, self.expression, self.outcome_name, self.obs_to_interprete, radius, n)
            if layer.shape[0] == 0:
                return np.array([])
            layer = self.is_valid(self.obs_to_interprete, layer, self.encoder, self.expression, self.outcome_name)
            ...
            if self.layer_shape == 'ring':
                ...
                layer = generate_ball(self.encoder, self.expression, self.outcome_name, self.obs_to_interprete, radius + step, n)
                if layer.shape[0] == 0:
                    return np.array([])
                layer = self.is_valid(self.obs_to_interprete, layer, self.encoder, self.expression, self.outcome_name)
    ...
    def feature_selection(self, counterfactual):
        ...
        for k in move_sorted:
            ...
            is_valid_enemy = self.is_valid(self.obs_to_interprete, [new_enn], self.encoder, self.expression, self.outcome_name)

            if condition_class and is_valid_enemy is not None:
                out[k] = new_enn[k]
                reduced += 1
