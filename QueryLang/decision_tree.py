...
class DecisionTree:
    def __init__(self, clf, encoder):
        ...
        self.leaves = [i for i in range(self.n_nodes) if self.is_leaf(i)]
        self.epsilon = 0.01

    def is_category(self, feature):
        return self.encoder.is_category(feature)
    ...
    def path(self,x):
        return self.path_from_node(0,x)
    ...
    def modify_to_value(self, original, modified_x, value, d=distance, k=3):
        modified_points = []
        for node_id in self.leaves:
            if self.node_prediction(node_id) == value:
                x_new = self.modify_to_match_a_leaf(modified_x, node_id)
                # TODO add distance base feature type
                changes = d(original, x_new)
                modified_points.append((changes, x_new))
        modified_points.sort(key=lambda x: x[0])
        modified_points = modified_points[:k]
        return [x[1] for x in modified_points]
    ...
    def modify_to_match_a_leaf(self, x, leaf):
        ...
        while node_id in self.parent:
            ...
            if x_new[feature] <= threshold and self.children_left[parent_id] != node_id:
                if self.is_category(feature):
                    x_new[feature] = self.encoder.closest(feature, threshold, bigger=True)
                else:
                    x_new[feature] = threshold + self.epsilon
            elif x_new[feature] > threshold and self.children_right[parent_id] != node_id:
                if self.is_category(feature):
                    x_new[feature] = self.encoder.closest(feature, threshold, bigger=False)
                else:
                    x_new[feature] = threshold
            node_id = parent_id
        return x_new
